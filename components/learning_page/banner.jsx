...
const RightSection = dynamic(() => import("../foreign-language-services/language-translation/content/RightSection"));
...
export default function LearningBanner(props) {
    ...
    useEffect(() => {
        let textt = (props?.data?.Banner?.content!=null) ? props.data.Banner.content : "";
        ...
        textt = textt.replace(/^- (.+)(\n- .+)*/gm, function (match, p1) {
            const listItems = match.trim().split('\n').map(item => `<li class=' flex  items-center'>${item.slice(2)}</li>`).join('\n');
    ...
    return (
        <div style={{backgroundImage:`url(${process.env.NEXT_PUBLIC_mainurl + props.data.Banner.image.data.attributes.url})`}} className="h-[601px]  w-full bg-cover flex  justify-center items-center">
            <div className="flex gap-[120px] justify-between">
                <div className="sm:w-[625px] w-[300px] px-[20px]  h-[273px] flex flex-col justify-center  bg-[#00000088] border-solid border-l-4 border-[#F60]" dangerouslySetInnerHTML={{__html:text}}>
